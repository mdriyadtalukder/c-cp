...
for bottom view, we do not need tracking visited index..er jnno nich theke dekhbo konta 1st e pore.r kongula convered.
*/
#include <bits/stdc++.h>
using namespace std;
...
// Find min & max horizontal distance--find left and right most
...
vector<int> topView(Node *root)
{
    ...
    queue<Node *> q;  // value thkbe..mane root ta
    queue<int> index; // value er index thkbe
    ...
    while (!q.empty())
    {
        ...
        {
            ans[pos] = temp->data; // ans er pos num index e value dilam
            filled[pos] = 1;       // so eita k visited krlm
        }
        // ans[pos] = temp->data; //it will be in bottom view
        if (temp->left)
        {
            q.push(temp->left);  // temp er left value queue te dukalam
            index.push(pos - 1); // index ta..ager index-1 hbe as its left side
        }
...
void Tview(Node *root, int pos, vector<int> &ans, vector<int> &level, int l)
{
    ...
    {
        ans[pos] = root->data;
        level[pos] = l; //pos er value jdi l er theke choto hole boshabo.level holo l and pos hoto value
    }
...
vector<int> topView(Node *root)
{
    ...
    vector<int> ans(r - l + 1);
    vector<int> level(r - l + 1, INT_MAX); // in bottom view it will be INT_MIN
